# Behaviors for Animats non-player characters in Second Life
...
## Introduction
This note describes how to write new behaviors for Animats NPCs. 
...
controlled by a separate script. A priority scheduler allows only one
behavior to control the NPC at a time.

Three standard behaviors are provided - "patrol" (bhvpatrolclassic, bhvpatrol), "greet" (bhvgreet),
and "avoid" (bhvavoid.) Patrol will visit a set of patrol points from a notecard. Greet will 
approach any new avatar and say hello. Avoid will get out of the way of vehicles. Patrol has
the lowest priority, and can be preempted by Greet. Greet, in turn, can be preempted by Avoid.

This combines the behaviors in a useful way. The NPC will visit patrol points until a new avatar
shows up, will go to that avatar and say hello, and will then return to patrolling. If a vehicle
is headed for an avatar, anything else going on will be stopped and the NPC will get out of the way,
then return to whatever else it was doing.

Behavior scripts don't need to know about other behavior scripts. This makes each behavior 
straightforward to write.

## What behavior scripts can do

Move the avatar and animate it, basically. Moving the avatar is done through the NPC
path planning system. The behavior can set a goal point or a target avatar, and the
path planning system will try to get the NPC to there. When it gets there, or fails
to get there, a callback in the behavior script is called.
